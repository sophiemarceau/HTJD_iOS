...
@implementation OrderDetaiWithContact
...
- (void)setData:(NSDictionary *)data {
    ...
    NSString *tranStr = [NSString stringWithFormat:@"%@",[data objectForKey:@"transportationFee"]];
    NSString *bedStr = [NSString stringWithFormat:@"%@",[data objectForKey:@"bedFee"]];
    if ([tranStr isEqualToString:@"0"] && [bedStr isEqualToString:@"0"]) {
        self.feeLabel.text = @"";
    }else if (![tranStr isEqualToString:@"0"] && [bedStr isEqualToString:@"0"]) {
        self.feeLabel.text = [NSString stringWithFormat:@"交通费￥%.2f",[[data objectForKey:@"transportationFee"]doubleValue]/100];
    }else if ([tranStr isEqualToString:@"0"] && ![bedStr isEqualToString:@"0"]) {
        self.feeLabel.text = [NSString stringWithFormat:@"带床上门￥%.2f",[[data objectForKey:@"bedFee"]doubleValue]/100];
    }else {
        self.feeLabel.text = [NSString stringWithFormat:@"带床上门￥%.2f  交通费￥%.2f",[[data objectForKey:@"bedFee"]doubleValue]/100,[[data objectForKey:@"transportationFee"]doubleValue]/100];
    }
...
//    }];
    ...
    if ([tranStr isEqualToString:@"0"] && [bedStr isEqualToString:@"0"]) {
        ...
        [self.totalTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.mas_left).offset(10);
            make.right.equalTo(self.mas_right).offset(-10);
            make.top.equalTo(self.feeLabel.mas_bottom).offset(10);
            make.height.mas_equalTo(14);
        }];
    }
...
- (UILabel *)feeLabel {
    if (_feeLabel == nil) {
        self.feeLabel = [CommentMethod initLabelWithText:@"带床上门￥20  交通费￥30" textAlignment:NSTextAlignmentRight font:14];
    }
    return _feeLabel;
}

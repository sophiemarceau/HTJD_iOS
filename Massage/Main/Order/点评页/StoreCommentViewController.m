...
@implementation StoreCommentViewController
{
    ...
    UIButton *storeScoreBtn;//门店评分按钮
    NSMutableArray *storeScoreBtnArray;//存储门店数组
    ...
    NSString *serviceScoreStr;
    NSString *workerScoreStr;
...
- (void)viewDidLoad {
    ...
    storeScoreBtnArray = [[NSMutableArray alloc]initWithCapacity:0];
...
- (void)initView {
    //门店评分五角星
    UILabel *storeLabel = [CommentMethod initLabelWithText:@"门店" textAlignment:NSTextAlignmentLeft font:15];
    storeLabel.frame = CGRectMake((kScreenWidth-210)/2, 15, 30, 15);
    storeLabel.textColor = BlackUIColorC5;
    [self.scoreView addSubview:storeLabel];
    
    storeScoreStr = @"5";//默认5分
    
    for (int i = 3000; i < 3005; i++) {
        storeScoreBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        storeScoreBtn.frame = CGRectMake(((kScreenWidth-210)/2+45) + (10+25)*(i-3000), 10, 25, 25);
        [storeScoreBtn setBackgroundImage:[UIImage imageNamed:@"icon_star_11"] forState:UIControlStateNormal];
        [storeScoreBtn addTarget:self action:@selector(storeScoreBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        storeScoreBtn.tag = i;
        [storeScoreBtnArray addObject:storeScoreBtn];
        [self.scoreView addSubview:storeScoreBtn];
    }
    //项目评分五角星
    UILabel *serviceLabel = [CommentMethod initLabelWithText:@"项目" textAlignment:NSTextAlignmentLeft font:15];
    serviceLabel.frame = CGRectMake((kScreenWidth-210)/2, 60, 30, 15);
    ...
    for (int i = 4000; i < 4005; i++) {
        ...
        serviceScoreBtn.frame = CGRectMake(((kScreenWidth-210)/2+45) + (10+25)*(i-4000), 55, 25, 25);
    ...
    workerLabel.frame = CGRectMake((kScreenWidth-210)/2, 105, 30, 15);
    ...
    for (int i = 5000; i < 5005; i++) {
        ...
        workerScoreBtn.frame = CGRectMake(((kScreenWidth-210)/2+45) + (10+25)*(i-5000), 100, 25, 25);
...
//        attitudeBtn.tag = 101 + i;
    ...
    if (self.view.bounds.size.height -64 - 10 - 142 - 10 - 10 - h - attitudeBtn.frame.size.height - 10 > 250) {
...
#pragma mark - 按钮点击事件
//门店评分
- (void)storeScoreBtnClick:(UIButton *)sender {
    for (storeScoreBtn in storeScoreBtnArray) {
        if (storeScoreBtn.tag <= sender.tag) {
            [storeScoreBtn setBackgroundImage:[UIImage imageNamed:@"icon_star_11"] forState:UIControlStateNormal];
        }else {
            [storeScoreBtn setBackgroundImage:[UIImage imageNamed:@"icon_star_22"] forState:UIControlStateNormal];
        }
    }
    storeScoreStr = [NSString stringWithFormat:@"%ld",(long)sender.tag-3000+1];
    NDLog(@"门店分数(到店):%@",storeScoreStr);
}
//项目评分
...
-(void)Show
{
    ...
    if (self.isFromFlashDeal == YES) {
        for (BaseViewController *vc in self.navigationController.viewControllers) {
            if ([vc isKindOfClass:[FlashDealViewController class]]) {

...
@interface FinishedHomeViewController () <UMSocialUIDelegate,UIAlertViewDelegate>

@end

@implementation FinishedHomeViewController
{
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 1) {
        if (buttonIndex == 0) {
            [self performSelector:@selector(gotoHomeVC) withObject:nil afterDelay:0];
        }
    }
    if (alertView.tag == 2) {
        if (buttonIndex == 0) {
...
-(void)gotoAppointVC:(NSDictionary *)dic
{
    ...
    //            [[RequestManager shareRequestManager] tipAlert:@"该门店已下线暂不可约,欢迎点选其他门店"  ];
...
//            [self performSelector:@selector(gotoHomeVC) withObject:nil afterDelay:2.0];
    ...
    if ([[DataDic objectForKey:@"workerName"]isEqualToString:@""]) {
        ...
        [[RequestManager shareRequestManager] getSysServiceDetail:dic viewController:self successData:^(NSDictionary *result) {
            ...
            if ([[result objectForKey:@"code"] isEqualToString:@"0000"]) {
                ...
                //                if ([result objectForKey:@"storeState"]&&![[NSString stringWithFormat:@"%@",[result objectForKey:@"storeState"]] isEqualToString:@""]) {
                //                    [[RequestManager shareRequestManager] tipAlert:@"门店已下架" viewController:self];
                //                    return ;
                //                }
                //                if ([result objectForKey:@"state"]&&![[NSString stringWithFormat:@"%@",[result objectForKey:@"state"]] isEqualToString:@""]) {
                //                    [[RequestManager shareRequestManager] tipAlert:@"项目已下架" viewController:self];
                //                    return ;
                //                }
                //                if ([result objectForKey:@"workerState"]&&![[NSString stringWithFormat:@"%@",[result objectForKey:@"workerState"]] isEqualToString:@""]) {
                //                    [[RequestManager shareRequestManager] tipAlert:@"技师已下架" viewController:self];
                //                    return ;
                //                }
                //
                //                DoorAppointmentFromServiceViewController *vc = [[DoorAppointmentFromServiceViewController alloc]init];
...
- (UIScrollView *)payScrollView {
    if (_payScrollView == nil) {
        ...
        //        self.payScrollView.contentSize = CGSizeMake(kScreenWidth, 726);

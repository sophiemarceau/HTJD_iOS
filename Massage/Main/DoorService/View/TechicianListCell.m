...
@implementation TechicianListCell
//@synthesize testArray,data;
...
//    TechicianView * abView = [[TechicianView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 380.0f*AUTO_SIZE_SCALE_X)];
//    abView.backgroundColor = [UIColor clearColor];
//    abView.data = self.data;
...
//    }];
    ...
    [self.backView mas_makeConstraints:^(MASConstraintMaker *make) {//背景图
        ...
        make.top.mas_equalTo(45*AUTO_SIZE_SCALE_X);
        make.bottom.mas_equalTo(self.mas_bottom);
    }];
    ...
    CGFloat dist =  [[NSString stringWithFormat:@"%@",[self.data objectForKey:@"distance"]] floatValue];
    if (dist >1) {
        self.distanceNumLabel.text = [NSString stringWithFormat:@"%0.1fkm",dist];

    }else{
        int distan =  dist*1000;
        self.distanceNumLabel.text = [NSString stringWithFormat:@"%dm",distan];

    }
    self.introduceLabel.text = [NSString stringWithFormat:@"%@",[self.data objectForKey:@"introduction"]];
    
    if ([[self.data objectForKey:@"storeName"] isEqualToString:@""]||![self.data objectForKey:@"storeName"]) {
        self.belongLabel.text = [NSString stringWithFormat:@"所属:%@",@"华佗驾到自营"];
    }
    else{
...
#pragma mark - 懒加载
//- (UIView *)contentView {
//    if (_contentView == nil) { \U5409\U65b0\U610f
...
- (UILabel *)yuYueButton {
    if (_yuYueButton == nil) {
        ...
        self.yuYueButton.layer.cornerRadius = 2;
